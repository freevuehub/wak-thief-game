...
const getStatusInfo = (thief: Thief, day: number): { text: string; color: string; disabled: boolean } => {
    if (thief.status === ThiefStatus.Arrested) return { text: '체포됨', color: 'text-yellow-400', disabled: true };
    if (thief.status === ThiefStatus.Executed) return { text: '처형됨', color: 'text-red-500', disabled: true };
    if (thief.recruitedOnDay === day) return { text: '대기 (신입)', color: 'text-cyan-400', disabled: true };
    if (thief.action !== undefined) return { text: '임무 배정됨', color: 'text-green-400', disabled: true };
