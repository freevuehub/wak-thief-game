        ...
        <header className="fixed top-0 left-0 right-0 bg-gray-900/80 backdrop-blur-sm p-3 z-20 border-b border-gray-700">
            <div className="max-w-7xl mx-auto flex justify-between items-center space-x-4">
                ...
                <div className="flex items-center space-x-4 md:space-x-6">
                    <StatItem icon={<MoneyIcon className="w-6 h-6 text-green-400"/>} label="자금" value={`$${cash.toLocaleString()}`} colorClass="text-green-300" />
                    <StatItem icon={<AlertIcon className="w-6 h-6 text-yellow-400"/>} label="경계" value={`${Math.round(globalAlertLevel)}%`} colorClass="text-yellow-300" />
                    <StatItem icon={<TeamIcon className="w-6 h-6 text-blue-400"/>} label="조직원" value={thiefCount} colorClass="text-blue-300" />
                    <StatItem icon={<LoyaltyIcon className="w-6 h-6 text-pink-400"/>} label="충성심" value={`${Math.round(averageLoyalty)}%`} colorClass="text-pink-300" />
                </div>
            </div>
        </header>
    );
