...
import { Thief, ThiefStatus } from '../legacyType'
...
const getStatusInfo = (
  thief: Thief,
  day: number
): { text: string; color: string; disabled: boolean } => {
  if (thief.status === ThiefStatus.Arrested)
    return { text: '체포됨', color: 'text-yellow-400', disabled: true }
  if (thief.status === ThiefStatus.Executed)
    return { text: '처형됨', color: 'text-red-500', disabled: true }
  if (thief.recruitedOnDay === day)
    return { text: '대기 (신입)', color: 'text-cyan-400', disabled: true }
  if (thief.action !== undefined)
    return { text: '임무 배정됨', color: 'text-green-400', disabled: true }
  return { text: '보고 대기중', color: 'text-gray-300 animate-pulse', disabled: false }
}
...
const ThiefRoster: React.FC<ThiefRosterProps> = ({ thieves, onSelectThief, day }) => {
  return (
    <div className="w-full h-full p-2 space-y-1 overflow-y-auto">
      <h3 className="font-display text-xl text-red-500 mb-2 px-2">조직원 명단</h3>
      {thieves.map((thief) => {
