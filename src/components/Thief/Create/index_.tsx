...
const ThiefCreation: React.FC<ThiefCreationProps> = ({ onThiefCreated, isFirstThief }) => {
  ...
  const [generatedThief, setGeneratedThief] = useState<Omit<Thief, 'id' | 'recruitedOnDay'> | null>(
    null
  )
    ...
    if (generatedThief) {
      return (
        <div className="flex flex-col items-center">
          <img
            src={generatedThief.portraitUrl}
            alt="Generated Thief"
            className="w-48 h-48 object-cover rounded-lg border-4 border-gray-600 mb-4"
          />
          <h3 className="text-2xl font-display">{generatedThief.name}</h3>
          <p className="text-gray-400">
            {generatedThief.personality} / {generatedThief.background}
          </p>
          <p className="italic text-center my-4 p-2 bg-black/20 rounded-md">
            "{generatedThief.dialogue.join(' ')}"
          </p>
          <div className="flex space-x-4 mt-4">
            ...
            <button
              ...
              className="px-6 py-2 bg-red-600 hover:bg-red-500 rounded-md transition-colors font-bold"
            >
              영입 확정
            </button>
          </div>
    ...
    return (
      <div className="space-y-4">
        <p className="text-center text-gray-400">
          새로운 조직원의 정보를 입력하세요. 비워두면 무작위로 생성됩니다.
        </p>
        <input
          type="text"
          value={name}
        ...
        <button
          onClick={handleGenerate}
          className="w-full mt-4 p-3 bg-red-600 hover:bg-red-500 rounded-md transition-colors font-bold"
        >
          생성
        </button>
      </div>
