...
const ConversationView: React.FC<ConversationViewProps> = ({
  ...
  const textToAnimate = (() => {
    switch (phase) {
      case 'opening':
        return thief.openingNarration || `${thief.name}이(가) 당신 앞에 섰다.`
      case 'dialogue':
  ...
  const renderContent = () => {
    if (phase === 'action') {
      return (
        <div className="w-full">
          <p className="text-center text-gray-400 mb-6 font-display text-xl">
            "지시를 내려주십시오, 보스."
          </p>
          <div className="grid grid-cols-2 gap-4">
            {Object.keys(ThiefAction)
              ...
              .map((actionKey) => {
                ...
                return (
                  ...
                  >
                    <h4 className="font-bold text-xl mb-1 text-red-400 group-hover:text-white">
                      {details.name}
                    </h4>
                    <p className="text-sm text-gray-400 group-hover:text-gray-200 flex-grow">
                      {details.description}
                    </p>
                  </button>
                )
    ...
    return (
      <div className="text-center">
        <p
          className={`text-2xl min-h-[6rem] ${isNarration ? 'text-gray-400 italic' : 'text-white'}`}
        >
          {animatedText}
        </p>
      </div>
  ...
  const buttonText = phase === 'closing' ? '면담 종료' : '계속...'

  return (
    <div className="fixed inset-0 bg-black/80 flex flex-col items-center justify-center z-40 backdrop-blur-sm p-4">
      <div
        className="bg-gray-900 border-2 border-red-500 p-8 rounded-lg shadow-2xl w-full max-w-4xl text-white relative flex flex-col"
        style={{ minHeight: '60vh' }}
      >
        <button
          onClick={onClose}
          className="absolute top-3 right-4 text-2xl font-bold text-gray-500 hover:text-white"
        >
          &times;
        </button>
        ...
        <div className="flex items-start space-x-8 mb-8">
          <div className="flex-shrink-0 text-center">
            <img
              ...
              alt={thief.name}
              className="w-48 h-48 object-cover rounded-lg border-4 border-gray-600"
            />
            <h2 className="mt-4 text-3xl font-display">{thief.name}</h2>
            <p className="text-gray-400">
              {thief.personality} / {thief.background}
            </p>
          </div>
          <div className="w-full mt-4">
            <div className="flex justify-around p-3 bg-gray-800/50 rounded-lg">
              <div className="text-center">
                <span className="text-sm text-gray-400">컨디션</span>
                <p className="text-2xl font-bold">{thief.condition}%</p>
              </div>
              <div className="text-center">
                <span className="text-sm text-gray-400">충성심</span>
