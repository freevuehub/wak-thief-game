...
type State = {
  stat: GameStat
  thieves: Map<string, Member>
  work: Map<string, Member>
  log: Array<GroupLog>
}
type Context = {
  state: State
...
const StoreProvider: React.FC<Props> = (props) => {
  ...
  // useEffect(() => {
  //   if (prompt[PROMPT_KEY.GENERATE_MAP] && areas.length === 0) {
  //     pipe('', syndicateAI.createMap(prompt[PROMPT_KEY.GENERATE_MAP].ko), ({ area }) => {
  //       console.log(area)
  //       setAreas(area)
      ...
      value={{
        ...
        ourMembers: useMemo(() => {
          return pipe(
            ...
            filter(({ team }) => team === MEMBER_TEAM.OUR),
            toArray
          )
        }, [state.thieves]),
        newMember: useMemo(() => {
          return pipe(
            [...state.thieves.values()],
            find(({ status }) => status === MEMBER_STATUS.RECRUITING)
          )
        }, [state.thieves]),
        ...
        updateDays: async () => {
          ...
          await pipe(
            ...
            replacePrompt(prompt[PROMPT_KEY.GENERATE_NEWS].ko),
        ...
        createThief: (thief) => {
          setState((prev) => {
            prev.thieves.set(thief.id, { ...thief, createdAt: prev.stat.day })
