    ...
    const handleEndDay = async () => {
            ...
            switch(thief.action) {
                ...
                case ThiefAction.Steal:
                    ...
                    if (Math.random() * 100 < successChance) {
                        ...
                        reportMsg = `${sector.name} 구역에서 $${loot.toLocaleString()}를 훔치는 데 성공했습니다.`;
                ...
                case ThiefAction.Scout:
                    ...
                    scoutedSector.scoutedInfo = `경계 ${Math.round(scoutedSector.alertLevel)}%, 예상 수익 $${scoutedSector.baseLoot.toLocaleString()}`;
        ...
        const dialogueUpdatePromises = updatedThieves.map((thief, index) => {
            if (thief.status !== ThiefStatus.Arrested && thief.status !== ThiefStatus.Executed) {
                 const eventSummary = dailyEventsForLog.find(e => e.message.includes(thief.name))?.message || "어제는 특별한 활동 없이 보냈습니다.";
            ...
            <div className="flex h-screen pt-20 pb-[68px]">
                <aside className="w-80 flex-shrink-0 bg-gray-800/50 border-r border-gray-700 flex flex-col">
                    ...
                    <div className="p-4 mt-auto border-t border-gray-700/50">
                        <button onClick={() => setShowRecruit(true)} className="w-full px-6 py-3 bg-blue-600 text-white hover:bg-blue-500 rounded-md transition-colors font-bold">새 조직원 영입</button>
                    </div>
                </aside>
                ...
                <main className="flex-1 overflow-y-auto p-8 flex items-center justify-center">
                    <div className="text-center text-gray-500">
                        ...
                        <p className="mt-2 max-w-md mx-auto">
                            {allTasksAssigned ? "하루를 종료하여 작전을 개시하십시오." : "좌측 명단에서 조직원을 선택하여 면담을 시작하고 임무를 부여하십시오."}
                        </p>
                    </div>
                </main>
            </div>
